...
const SmTreatmentPaymentComp = ({ payment,getAppointments }) => {
    ...
    return (
        <div className='rounded-md py-[10px] px-3 bg-[#F4F9EA] w-[154px] flex flex-col items-center gap-5'>
            <div className='flex gap-3 justify-start items-center w-full -mb-2'>
                ...
                <p className='text-[#1A1A1A] text-xs font-normal font-inter'>{payment?.date && format(new Date(payment?.date), "dd-MM-yyyy")}</p>
            </div>
            ...
            <div className='flex gap-3 justify-start items-center w-full'>
                ...
                <p className={`${payment?.paymentStatus === "paid" ? "text-[#00CD4B]" : "text-[#FF0000]"} text-sm capitalize font-normal font-inter`}>{payment?.paymentStatus}</p>
            </div>
...
export default SmTreatmentPaymentComp