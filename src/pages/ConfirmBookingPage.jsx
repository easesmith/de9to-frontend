        ...
        <Layout>
            <div className='grid grid-cols-2 max-[900px]:grid-cols-1 gap-5 max-w-[1240px] mx-auto mb-5'>
                <div className='bg-[#FCFFF4] px-4 py-6'>
                    <h1 className='font-inter font-medium text-xl sm:text-3xl'>In Clinic Appointment</h1>
                    <h3 className='mt-5 font-inter text-base sm:text-xl'>Appointment Details</h3>
                    <div className='flex justify-between items-start mt-5'>
                        <div className='flex items-center gap-2'>
                            <FaRegCalendarAlt className='text-[#717171]' />
                            <span className='font-inter text-sm sm:text-base text-[#1A1A1A]'>On {format(new Date(selectedDate), "EEE MMM dd,yy")}</span>
                        </div>
                        <div>
                            <div className='flex items-center gap-2'>
                                <FaRegClock className='text-[#717171]' />
                                <span className='font-inter text-sm sm:text-base text-[#1A1A1A]'>At {selectedTime}</span>
                            </div>
                            <Button onClick={() => setIsConfirmBookingModalOpen(true)} variant="outline" className="h-7 text-[#95C22B] text-sm sm:text-base hover:text-[#95C22B] mt-4">Change Date & Time</Button>
                        </div>
                    </div>

                    <h4 className='font-inter mt-4 text-[#1A1A1A] text-sm sm:text-base font-semibold'>Selected Dentist</h4>
                    ...
                    <h4 className='font-inter mt-4 text-[#1A1A1A] text-sm sm:text-base font-semibold'>Selected Clinic</h4>
                    <div className='shadow-lg rounded-md p-3 mt-5 bg-white grid max-[500px]:grid-cols-1 grid-cols-[40%_58%] lg:grid-cols-[30%_68%] gap-3'>
                        ...
                        <div>
                            ...
                            <div className="flex gap-2 items-center mt-3">
                                ...
                                <h5 className='text-[#717171] text-base max-[500px]:text-sm font-semibold font-inter'>Multi-Speciality Clinic</h5>
                            </div>
                            <p className='font-inter text-sm max-[500px]:text-xs text-[#717171] my-2'><span className='font-bold'>Address:</span>{`${clinicAddress}, ${nearbyLandmark}, ${city}, ${state}, ${clinicPincode}`}</p>
                            <div className='flex flex-wrap items-center gap-4'>
                                <Button variant="outline" size="sm" className="flex max-[500px]:text-sm max-[500px]:h-8 max-[500px]:px-2 gap-2 text-[#95C22B] border-[#95C22B] hover:text-[#95C22B]">
