// import { Input } from '@/components/ui/input'
import React from 'react'

const LocationCompo = ({ searchQuery, setSearchQuery, handleGetSerachQuery, location , isShadow ,setLocation}) => {
    
    return (
        <div className={` bg-[#FFFFFF] flex justify-center items-center gap-5 ${isShadow?'rounded-b-none border-t-0 border-s-0 border-e-0':''} rounded-2xl p-8 border-[1px] border-[#D9D9D9]`}>
            <div className=' bg-[#FFFFFF] border-[1px] border-[#E1E1E1] rounded-2xl'>
                <input
                    type="search"
                    value={searchQuery}
                    onChange={(e)=> setSearchQuery(e.target.value)}
                    placeholder='Search doctors, clinic etc.'
                    className="w-[500px] text-[#000000] font-medium font-poppins rounded-2xl placeholder:text-[#000000] px-10 py-5"
                />
            </div>
            <div className='  bg-[#FFFFFF] border-[1px] border-[#E1E1E1] rounded-2xl px-4 py-5'>
                <select value={location} onChange={(e)=> setLocation(e.target.value)} className="w-[228px] border-none outline-none text-[#3F3F3F] font-medium font-poppins">
                    <option disabled value="">location</option>
                    <option>Delhi</option>
                    <option>Lucknow</option>
                    <option>Maharashta</option>
                    <option>ABC</option>
                    <option>Maggie</option>
                </select>
            </div>
            <div className=' '>
                <button onClick={handleGetSerachQuery} className=' w-[228px] bg-[#95C22B] text-[#FFFFFF] text-xl font-semibold font-poppins border-[1px] border-[#95C22B]  rounded-lg px-10 py-4'>search</button>
            </div>
        </div>
    )
}

export default LocationCompo